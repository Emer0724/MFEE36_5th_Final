  ...
  return (
    <>
      <div className={`${style.chennav} justify-content-between pb-3 pt-3`}>
        <div>
          <Link
            className={`text-decoration-none fw-bold text-black ${activeNavItem === '部落格' ? 'active' : ''}`}
            href="http://localhost:3000/blog/blog-home-page"
            onClick={() => handleNavItemClick('部落格')}
          >
            {activeNavItem === '部落格' ? <BiSolidBook className="pb-1" /> : <BiBook className="pb-1" />}
            部落格
          </Link>
        </div>
        <div>
          <Link
            className={`text-decoration-none fw-bold text-black ${activeNavItem === '書評' ? 'active' : ''}`}
            href="http://localhost:3000/blog/book-review"
            onClick={() => handleNavItemClick('書評')}
          >
            {activeNavItem === '書評' ? <FaPencilAlt className="pb-1" /> : <BsPencil className="pb-1" />}
            書評
          </Link>
        </div>
        <div>
          <Link
            className={`text-decoration-none fw-bold text-black ${activeNavItem === '我的' ? 'active' : ''}`}
            href="http://localhost:3000/blog/personal-page/work"
            onClick={() => handleNavItemClick('我的')}
          >
            {activeNavItem === '我的' ? <GoPersonFill className="pb-1" /> : <GoPerson className="pb-1" />}
            我的
          </Link>
        </div>
        <div>
